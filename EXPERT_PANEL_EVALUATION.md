...
The technique described here adds a second relationship: **expert adversary**. You ask the LLM to evaluate what you've built, not build more of it. The difference in output quality can be striking.
...
This is not a trick. It reflects something real: the framing you give an LLM changes which patterns it matches against, which failure modes it prioritizes, and how it weights trade-offs.
...
If the codebase is large, start with the highest-level files and the invariants. You can drill down per-expert in later passes.
...
Save this document. It is your implementation specification for the next phase.

### Step 5: Generate an Implementation Plan

Now switch back to builder mode. Give the coding agent the consolidated findings document and ask it to produce a phased implementation plan.
...
LLMs are prone to sycophancy. An expert asked to review your system may find the exercise gratifying and produce an unrealistically balanced report. A few techniques counter this:
...
## Integration with the Rest of This Guide

This technique fits naturally at the end of major phases, before starting new ones. Specifically:
...
* **Before production deployment**: run an adversarial panel focused on security and failure modes
* **After significant growth** (the codebase has doubled in size): run a systems-level review

Store the consolidated findings documents in `/docs`. They become part of the project's institutional memory and are useful context for future sessions.
